...
func main() {
    ...
    value := reader.U16R(12, 0) // returns 0b101011001110
    
    // Read 4 bits from position 2
    value2 := reader.U8R(4, 2)  // returns 0b1110
}
```
...
func main() {
    ...
    writer.U16(0, 12, 0b101011001110)
...
- `U8R(bits, n int) uint8` - Read up to 8 bits
...
- `U8(leftPadd, bits int, data uint8)` - Write up to 8 bits
- `U16(leftPadd, bits int, data uint16)` - Write up to 16 bits
- `U32(leftPadd, bits int, data uint32)` - Write up to 32 bits
