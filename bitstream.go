...
type BitReader[T IntegerType] struct {
	...
	s    int // Number of valid bits per element (element size - left padding)
...
// For example, if each element has 2 bits of padding at the top, set leftPadd=2.
// The reader will only access bits from position leftPadd to the end of each element.
func NewBitReader[T IntegerType](data []T, leftPadd int) *BitReader[T] {
	var zero T
	var s = int(unsafe.Sizeof(zero))*8 - leftPadd
	return &BitReader[T]{
		data: data,
		bits: len(data) * s,
		s:    s,
...
// U16 reads a specified number of bits from the n-th position in the data.
// bits specifies how many bits to read (up to 16 bits).
// n specifies which block to read (0-indexed).
// Returns the bits as a uint16 value, MSB-aligned.
func (r BitReader[T]) U16(bits, n int) (b uint16) {
	s := min(n*bits, r.bits)
	e := min(s+bits, r.bits)
	for i := s; i < e; i++ {
		b <<= 1
